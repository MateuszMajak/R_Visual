...
### Drivers difference
```{r}
top3drivers <- results_drivers %>% filter(position %in% c(1, 2, 3))

# top3drivers$year_points <- (top3drivers %>%
#   dplyr::group_by(driverRef, year) %>%
#   dplyr::summarise(year_points = sum(points)))$year_points

# ggplot(data=top3drivers,aes(x=year,y=points,color=position)) +
#     geom_point()+
#     geom_line(aes(group=year),size=1) +
#     scale_x_continuous(breaks=seq(1950,2020,10)) +
#     scale_y_continuous(breaks=seq(0,max(results_drivers$points),40)) +
#     labs(title='Difference of points between 1st, 2nd and 3th place',
#         subtitle='Most of the years the first place has won the tournament by a large margin.',
#         y='Points',color='Position')
```
